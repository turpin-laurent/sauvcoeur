...
// ── Page intérieure ───────────────────────────────────────────
function AnimauxPageInner({ initialAnimals }: { initialAnimals: PublicAnimal[] }) {
  ...
  // Données serveur injectées directement — pas de fetch client
  const allAnimals = initialAnimals
  ...
  const filtered = useMemo(() => allAnimals.filter(a => {
    if ((a as any).moderation_status !== 'approved') return false
...
export default function AnimauxClient({ initialAnimals }: { initialAnimals: PublicAnimal[] }) {
  return (
    <Suspense fallback={<div className="p-8 text-center text-slate-400">Chargement…</div>}>
      <AnimauxPageInner initialAnimals={initialAnimals} />
    </Suspense>
  )
}
