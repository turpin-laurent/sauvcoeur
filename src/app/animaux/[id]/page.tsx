...
import { MapPin, Heart, Clock, ArrowLeft, MessageCircle, PawPrint, CheckCircle2, X, ChevronLeft, ChevronRight, Phone, Send, Calendar, Navigation } from 'lucide-react'
...
export default function AnimalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mainIdx,    setMainIdx]    = useState(0)           // photo principale affichée
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6 sm:py-8 space-y-5">
      ...
      <div className="space-y-2">
        {/* Photo principale — clic = zoom lightbox */}
        ...
        {/* Miniatures — clic = changer la photo principale (pas de zoom) */}
        {photos.length > 1 && (
